...
var fieldElementsAmounts = {dots: 0, circles: 0, arcs: 0, files: 0};
var mode = modes.dot;
// var IMPORTANT_MASTER_LIST = {1: structuredClone(___tableList)}
...
function newSetMousePosition(e) {
  ...
  for (let i = 0; i < ___tableList.length; i++) {
    if ((___tableList[i][0] == mouseX) && (___tableList[i][1] == mouseY)) {
        if (confirm("Do you want to edit this itsm's position? If not, click cancel.")) {
          _bad =structuredClone(___tableList[i]);
        } else {
          _bad = true;
        }
    }
  ...
  if (_bad == false) {
      ...
      if (mode == modes.arc) {
        fieldElementsAmounts.arcs = fieldElementsAmounts.arcs + 1;
        _amount = structuredClone(fieldElementsAmounts.arcs);
      } else if (mode == modes.circle) {
        fieldElementsAmounts.circles = fieldElementsAmounts.circles + 1;
        _amount = structuredClone(fieldElementsAmounts.circles);
      } else if (mode == modes.dot) {
        fieldElementsAmounts.dots = fieldElementsAmounts.dots + 1;
        _amount = structuredClone(fieldElementsAmounts.dots);
      } else if (mode == modes.file) {
        fieldElementsAmounts.files = fieldElementsAmounts.files + 1;
        _amount = structuredClone(fieldElementsAmounts.files);
      }
      if (mode == modes.circle) {
        ...
        if (conditionMet == null) {
          ...
          ___tableList.push([_mouseX, _mouseY, _amount, mode + _amount, cordX, cordY, _cell4, mode, [radius], [], "_CGcirlceElement_" + _amount]);
        }
      } else if (mode == modes.dot) {
        ...
        ___tableList.push([_mouseX, _mouseY, _amount, mode + _amount, cordX, cordY, _cell4, mode, null, null]);
      }
  } else {
...
function drawPointsOnCircle(_item) {
  // [_mouseX, _mouseY, structuredClone(___tableList.length + 2), mode + _amount, cordX, cordY, _cell4, mode, null, null]
  _dpoctemp = new Array();
  ...
  for (let z = 0; z < _item[9].length; z++) {
    if (_dpoctemp.includes("_CGmarcherElement_" + _item[9][z][2]) == false) {
      _item[9].pop(z);
    }
  }
  for (let z = 0; z < _item[9].length; z++) {
...
function draw() {
    ...
    context.stroke();
    for (let i = 0; i < ___tableList.length; i++) {
        if (___tableList[i][10] != false) {
          context.beginPath();
        context.arc(___tableList[i][0], ___tableList[i][1], (1/360)*width, 0, 2 * Math.PI);
        if (___tableList[i][7] == modes.dot) {
          context.fillStyle = "#000080";
        } else if (___tableList[i][7] == modes.circle) {
          context.fillStyle = "#8a0000";
        } else if (___tableList[i][7] == modes.arc) {
          context.fillStyle = "#ffffff";
        } else if (___tableList[i][7] == modes.file) {
          context.fillStyle = "#4300a1";
        }
        context.fill();
        if (___tableList[i][7] == modes.circle) {
          context.beginPath();
          context.arc(___tableList[i][0], ___tableList[i][1], (___tableList[i][8][0]/360)*width, 0, 2 * Math.PI);
          context.stroke();
          drawPointsOnCircle(___tableList[i]);
        } else if (___tableList[i][7] == modes.arc) {
...
function drop(ev) {
  ...
  if (ev.target.id.startsWith('_CGmarcherElement') == false) {
    ...
    for (let i = 0; i < ___tableList.length; i++) {
      if (___tableList[i][3] == _element.innerHTML) {
        if (ev.target.id.startsWith('marcherManagementDiv')) {
          ___tableList[i][10] = true;
        } else {
          ___tableList[i][10] = false;
        }
